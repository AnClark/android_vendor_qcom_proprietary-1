...
int ditherTest(void){
	...
	overlay.dst_rect.x = 0;
	overlay.dst_rect.y = 0;
        overlay.dst_rect.w = (thisFBTEST->fileParams.inResolution.width < FB->fb_vinfo.xres) ?
                thisFBTEST->fileParams.inResolution.width : FB->fb_vinfo.xres;
        overlay.dst_rect.h = (thisFBTEST->fileParams.inResolution.height < FB->fb_vinfo.yres) ?
                thisFBTEST->fileParams.inResolution.height : FB->fb_vinfo.yres;
	overlay.z_order = FB_OVERLAY_RGB_1;
	overlay.alpha = 0xFF;
	if (FB->fb_vinfo.bits_per_pixel == 32) {
		overlay.is_fg = 0;
...
void *doScaleptr(void* arg)
{
    ...
    for(width = src_min_width; width <= ref_xres; width += width_increment) {
		...
		overlay.alpha= 0xFF;
		tmp_result = ioctl(FBLIST_GLOBAL[fcount]->fb_fd, MSMFB_OVERLAY_SET, &overlay);
