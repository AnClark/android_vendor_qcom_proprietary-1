...
public class QSecureElementManager extends IQNfcSecureElementManager.Stub {
    ...
    public QSecureElementManager(Application nfcApplication, DeviceHost host, NfcService nfcService) {
        ...
        if (mDeviceHost.isMultiSeEnabled())
            mMultiSeManager = new MultiSeManager(mContext, mNfcService.mCardEmulationManager);
    }
                        ...
                        if (receptionMode == multiReceptionMode) {
                            ...
                            try {
                                if (mIsSentUnicastReception == false) {
                                    try {
                                        seIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                                        mNfcService.mContext.startActivity(seIntent);
                                    } catch (Exception e) {
                                    }
                                    //start gsma
...
//        NfcService.this.enforceNfceeAdminPerm(pkg);
    ...
    public static QSecureElementManager getInstance() {
        if (DBG) Log.d(TAG, "QSecureElementManager getInstance");
        return sService;
    }
