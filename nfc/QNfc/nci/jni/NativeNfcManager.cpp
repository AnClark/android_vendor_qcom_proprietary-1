...
static bool                 sPollRfActive = false;  // Poll mode is activated
...
#define ENABLE                      1
#define DISABLE                     0
void DTA_modifyNVM(UINT32 index,bool action_flag);
#endif // </DTA>
...
bool  sUiStateNeedCallback = true;
...
static void nfaConnectionCallback (UINT8 connEvent, tNFA_CONN_EVT_DATA* eventData)
{
    ...
    case NFA_ACTIVATED_EVT: // NFC link/protocol activated
        ...
        else if (pn544InteropIsBusy() == false)
        {
            ...
            {
                sSeRfActive = true;
                // if payment transaction then notify HostCardEmulation
                if (NFC_INTERFACE_EE_DIRECT_RF == eventData->activated.activate_ntf.intf_param.type)
                {
                    RoutingManager::getInstance().notifyActivated ();
                }
                else
                {
    ...
    case NFA_DEACTIVATED_EVT: // NFC link/protocol deactivated
                ...
                || (eventData->deactivated.type == NFA_DEACTIVATE_TYPE_DISCOVERY))
        {
            if (sSeRfActive) {
                ...
                if (!sIsDisabling && sIsNfaEnabled)
                {
                    //notify HostCardEmulation to complete tapping again in case that default app is not selected
                    RoutingManager::getInstance().notifyDeactivated ();
                }
            } else if (sP2pActive) {
                sP2pActive = false;
...
#endif // </DTA>
                if (eventData->status == NFA_STATUS_SYNTAX_ERROR) {
                    NFA_NFC_Deactivate(TRUE);
...
{
    ALOGD ("%s: enter; gActivated=%d, sSeRfActive=%d", __FUNCTION__, gActivated, sSeRfActive);
...
static void nfcManager_updateHostPresence(JNIEnv*, jobject, jint hciUiState, jint nfccUiState)
{
    ...
    ALOGD ("%s: hciUiState=%d, nfccUiState=%d", __FUNCTION__, hciUiState, nfccUiState);
    if (sIsNfcRestart == true)
    {
        UINT8  p_param[] = {0x4F, 0x01, 0x01, 0x00 };
        ALOGD ("%s in restart, announce fake ui rsp", __FUNCTION__);
    ...
    if ((sPollRfActive == true) &&
        ((nfccUiState == NCI_PROP_NFCC_UI_STATE_LOCKED) || (nfccUiState == NCI_PROP_NFCC_UI_STATE_OFF)))
    {
        NFA_Deactivate (FALSE);
    }
...
void startRfDiscovery(bool isStart)
{
    ...
    switch (sDtaPatternNumber) {
     ...
     //   UINT8 nfc_f_condevlimit_param[] = { 0x00 };
      ...
      case 0x2B: { // 43 for Poll B w. Sleep Response (copy of 0x00)
...
void doStartupConfig()
{
...
#ifdef DTA // <DTA>
    // Set pattern again in NFA thread, enforcing configuration
    stat = NFA_DTA_Set_Pattern_Number(NFA_DTA_Get_Pattern_Number());

    ALOGD("%s: sNfcChipVersion = %d, in_dta_mode(): %d", __FUNCTION__,
            sNfcChipVersion, in_dta_mode());
    ...
    if (sNfcChipVersion >= 30)
    {
        ...
        if(in_dta_mode())
        {
            DTA_modifyNVM(RFPOLLMACP_ENABLE_FEATURE,DISABLE);
            if (!GetNumValue("AGC_DISABLE_MASK", &num, sizeof(num)))
            {
                ALOGD("%s: disabling AGC", __FUNCTION__);
                DTA_modifyNVM(RXAGCMAP_RXAGCENABLE,DISABLE);
            }
        }
        else
        {
             ALOGD("%s: Enabling AGC", __FUNCTION__);
            if (!GetNumValue("AGC_DISABLE_MASK", &num, sizeof(num)))
            {
                DTA_modifyNVM(RXAGCMAP_RXAGCENABLE,ENABLE);
            }
        }
    }
#endif // </DTA>
...
/******************************************************************************
 **
 ** Function:        DTA_modifyNVM
 **
 ** Description:     read NVM from NFCC, modify NVM accordingly
 **                  write NVM to NFCC
 **                  currently only NVM 155: RFPOLLMACP_ENABLE_FEATURE
 **                  extendible for other tag-indexed NVMs
 **
 ** Returns:         None.
 **
 ******************************************************************************/
void DTA_modifyNVM(UINT32 index, bool action_flag) {
    ...
    if(index == RFPOLLMACP_ENABLE_FEATURE) {
        /*
         * NVM 155: RFPOLLMACP_ENABLE_FEATURE
         * disable ENABLE_RF_POLL in dta mode
         */
        sNVM_155 sNVM_155_storage;
        sNVM_155_storage.raw = nvm[3]; // result is in nvm[3]
        sNVM_155_storage.reg.ENABLE_RF_POLL = 0;
    ...
    else if(index == RXAGCMAP_RXAGCENABLE)
    {
        ...
        if(action_flag == DISABLE)
        {
            sNVM_1763_storage.reg = 0x00;       // disable AGC
        }
        else
        {
            sNVM_1763_storage.reg = 0x01;       // enable AGC
        }
