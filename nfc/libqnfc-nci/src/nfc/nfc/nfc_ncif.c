...
void nfc_ncif_check_cmd_queue (BT_HDR *p_buf)
{
    ...
    if (nfc_cb.nci_cmd_window > 0)
    {
        ...
        if (p_buf)
        {
            ...
            if (nfa_dm_cb.req_screen_off_lock != 0xFF)
            {
                nfa_dm_cb.req_screen_off_lock = 0xFF;
                /* screen off/lock cmd was sent */
            }
            ps = (UINT8 *)(p_buf + 1) + p_buf->offset;
            ...
            if (!fragmented &&
                ...
                (ps[2] == 0x03) &&
                (ps[3] == NCI_MSG_PROP_GENERIC_SUBCMD_UI_STATE))
            {
                if ((ps[5] == NCI_PROP_NFCC_UI_STATE_OFF) ||
                   (ps[5] == NCI_PROP_NFCC_UI_STATE_LOCKED) )
                {
                    nfa_dm_cb.req_screen_off_lock = ps[5];
                }
                NFC_TRACE_DEBUG0 ("NCI_MSG_PROP_GENERIC_SUBCMD_UI_STATE, no response from NFCC, but fake response from HAL");
            }
