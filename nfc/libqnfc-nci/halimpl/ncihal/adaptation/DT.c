        ...
        if (rx_length > 0)
        {
            ...
            if (p1 != NULL)
            {
                ...
                if ((mt == NCI_MT_NTF) && (gid == NCI_GID_RF_MANAGE))
                {
                    ...
                    {
                       if ((*(p1 + 1) != NCI_INTERFACE_EE_DIRECT_RF) &&
                           (((*(p1 + 3)) & 0x80 )== 0x00))
                        {
                            if (nfc_hal_cb.sent_uiofflocked == TRUE)
                            {
...
UDRV_API UINT16  DT_Nfc_Write(tUSERIAL_PORT port, UINT8 *p_data, UINT16 len)
{
    ...
    if((p_data[0] == NCI_PROP_CMD) &&
            ...
            (p_data[3] == NCI_MSG_PROP_GENERIC_SUBCMD_UI_STATE))
    {
        HAL_TRACE_DEBUG3 ("Got screen state cmd,%d,%d,%d" , p_data[5], nfc_hal_cb.poll_activated_event, nfc_hal_cb.sent_uiofflocked);
         /*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
        if (nfc_hal_cb.poll_activated_event == TRUE)
        {
            if ((p_data[5] == NCI_PROP_NFCC_UI_STATE_OFF) ||
        ...
        {
            if (p_data[5] == NCI_PROP_NFCC_UI_STATE_UNLOCKED)
            {
                ...
                nfc_hal_cb.sent_uiofflocked = FALSE;
            }
            else
            {
                nfc_hal_cb.sent_uiofflocked = TRUE;
            }
        }
        /*  QNCI_FEATURE_UI_SCREEN_ERR_HANDLE */
    }
