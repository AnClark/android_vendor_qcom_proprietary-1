...
public class QcRilHook implements IQcRilHook {
    ...
    // FUNCTION: qcRilGetConfig
    //
    // DESCRIPTION: queries RIL for current configuration in SUB0
    //
    // RETURN: string - the current configuration or null
    //========================================================================
    ...
    //        sub          sub id that needs to be verified
    //
    // RETURN: true if send commands to modem successfully, otherwise false
    //========================================================================
    public boolean qcRilValidateConfig(String configId, int sub) {
        if (configId.isEmpty() || configId.length() > MAX_PDC_ID_LEN) {
            Log.w(LOG_TAG, "invalid config id");
            return false;
        }
    ...
    public boolean qcRilSetLteTuneaway(int enable, int sub) {
        boolean retval = false;
    ...
    public void qcRilSendApnInfo(String type, String apn, int phoneId) {
        ...
        int payloadSize = 2 * INT_SIZE + type.length() + apn.length() + 2 * BYTE_SIZE;
