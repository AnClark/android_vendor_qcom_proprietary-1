...
int diag_register_dci_client (int *client_id, diag_dci_peripherals *list, int channel, void *os_params)
{
	...
	dci_client_tbl->client_info.notification_list = *list;
	dci_client_tbl->client_info.signal_type = *(int *)os_params;
	dci_client_tbl->data_signal_flag = DISABLE;
	dci_client_tbl->data_signal_type = DIAG_INVALID_SIGNAL;

	*client_id = ioctl(fd, DIAG_IOCTL_DCI_REG, &dci_client_tbl->client_info, 0, NULL, 0, NULL, NULL);

	if (*client_id == DIAG_DCI_NO_REG) {
		DIAG_LOGE(" could not register client, error: %d\n", errno);
	} else {
		dci_client_tbl->client_info.client_id = (unsigned int)*client_id;
...
int diag_release_dci_client(int *client_id)
{
	...
	if (result == -1) {
		...
		*client_id = 0;
		dci_client_tbl->client_info.client_id = 0;
...
int diag_get_health_stats(struct diag_dci_health_stats *dci_health)
{
	...
	health_proc.health = dci_health;
	health_proc.proc = DIAG_ALL_PROC;

	return ioctl(fd, DIAG_IOCTL_DCI_HEALTH_STATS, &health_proc, 0, NULL, 0, NULL, NULL);
}
