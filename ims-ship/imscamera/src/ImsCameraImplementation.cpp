...
void ims_camera_notify_callback(int32_t msgType,
					int32_t ext1,
					int32_t ext2,
					 void* user)
{
	 ALOGE("imscamera::ims_camera_notify_callback msgType = %d",msgType);
	 ImsCameraClient *ptr = ImsCameraClient::getInstance();
	 if(global_imscamera_cb.notify_cb)	{
		global_imscamera_cb.notify_cb(msgType,ext1,
					ext2,
					user);
	 }
}

 void ims_camera_data_callback(int32_t msgType,
					 const sp<IMemory> &dataPtr,
					 camera_frame_metadata_t *metadata,
					  void* user)
 {
	 ALOGE("imscamera::ims_camera_data_callback size : %d msgType : %d",dataPtr->size(),msgType);
	 ImsCameraClient *ptr = ImsCameraClient::getInstance();

	 if(global_imscamera_cb.data_cb){
		global_imscamera_cb.data_cb(msgType,dataPtr,
					metadata,
					user);
	 }
 }
void ims_camera_data_callback_timestamp(nsecs_t timestamp,
					int32_t msgType,
					const sp<IMemory> &dataPtr,
					 void* user)
{
	ImsCameraClient *ptr = ImsCameraClient::getInstance();
	if(global_imscamera_cb.data_cb_timestamp) {
		...
		if(stopping != 1)
			{
				executeCommand(RELEASE_CAMERA_FRAME,(void*)(&dataPtr),0,0);
			}
		else
			{
				int temp = (processFrameIndex + processFrameCount)%TEMP_CAMERA_BUFFER_MAX;
				framePointer[temp] = (void*)&dataPtr;
				ALOGE("camera buffer to be released %p %d %d",&dataPtr,temp,processFrameIndex);
				processFrameCount++;
			}
