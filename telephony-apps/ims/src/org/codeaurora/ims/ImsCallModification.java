...
public class ImsCallModification {
    ...
    private boolean validateCanModifyConnectionType(Message msg, int newCallType) {
        boolean ret = false;
        int currCallType = mImsCallSessionImpl.getInternalCallType();

        if (DBG) {
            log("validateCanModifyConnectionType currCallType= " + currCallType + " newCallType="
                    + newCallType);
        }

        CallDetails callDetails = new CallDetails(newCallType, CallDetails.CALL_DOMAIN_AUTOMATIC,
                null);
        final int videoPauseState = callDetails.getVideoPauseState();
        mIndex = Integer.parseInt(mImsCallSessionImpl.getCallId());
        ret = (mIndex >= 0 && (newCallType != currCallType
                || videoPauseState == CallDetails.VIDEO_PAUSE_STATE_PAUSED));

        log(" mIndex=" + mIndex + " newCallType=" + newCallType + " Current callType="
                + callDetails.call_type + " VideoPauseState=" + videoPauseState);
        if (!ret && (msg != null)) {
            msg.obj = AsyncResult.forMessage(msg, null, new Exception("Can't modify call."));
            msg.sendToTarget();
        }
        return ret;
    }
