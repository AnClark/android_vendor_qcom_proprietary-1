...
public class DataStatusNotificationService extends Service {
    ...
    private void enableContentObservers() {
        ...
        for (int i = 0; i < mPhoneCount; i++) {
            ...
            // Explicitly trigger onChange() at bootup/start of service since modem needs to know
            // the values at bootup as well
        ...
        if (operator != null && !operator.isEmpty()) {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                ...
                if ((newApn = getPreferredApn(subId, selection)) != null) {
                    ...
                    Log.d(TAG,
                            "Could not find preferred or default APN. Bail out");
                    cursor.close();
                    return;
                }
                // Call oemhook with newApn if it is not same as previously stored apn
                if (!mInternetApns[phoneId].equals(newApn.getApn())) {
                    if (DBG)
                        Log.d(TAG, "Found new apn.Call oemhook on phoneId " + phoneId);
                    mInternetApns[phoneId] = newApn.getApn();
                    mQcRilHook.qcRilSendApnInfo(newApn.getType(), newApn.getApn(), phoneId);
                }
                cursor.close();
            } else
                Log.d(TAG,
                        "No rows in Carriers DB for current operator.Bail out.");
        } else
            Log.d(TAG, "Could not get current operator. Bail out");
    }

    private Apn getApnFound(Cursor cursor) {
        Apn apn = null;
