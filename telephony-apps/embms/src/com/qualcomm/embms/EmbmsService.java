...
public class EmbmsService extends Service {
    ...
    private static final String VERSION = "00.08.01";
    ...
    private final IEmbmsService.Stub mBinder = new IEmbmsService.Stub() {
        ...
        /**
         * Utility to the get the system time, that is, time present in the ATS_USER offset.
         * The time is retrieved locally (that is, request is not sent to RIL) and the result
         * is posted back to EmbmsService.
         */
        private void getSystemTime(int traceId, Message msg) {
            int currentLocalOffsetInMins;
            long timeMseconds;
            long localTimeOffset;
            boolean additionalInfo;
            boolean dayLightSaving;
            byte leapSeconds = 0;
            Message message = Message.obtain(msg.getTarget(), msg.what);

            //TimeZone.getDefault() returns the user's preferred time zone.
            TimeZone currentTimeZone =  TimeZone.getDefault();
            // Initializes the Date instance to the current time , that is,
            // System.currentTimeMillis()
            Date currentDate = new Date();
            timeMseconds = currentDate.getTime();
            currentLocalOffsetInMins = currentTimeZone.getOffset(timeMseconds) / (1000 * 60);
            //The localTimeOffset is represented in 15 minute increments; For example, if we are
            // off by an hour, the offset is 60/15 = 4.
            localTimeOffset = (long) (currentLocalOffsetInMins / 15);
            additionalInfo = true;
            dayLightSaving = currentTimeZone.inDaylightTime(currentDate);
            message.obj = mEmbmsOemHook.new TimeResponse(traceId, SUCCESS, timeMseconds,
                    additionalInfo, localTimeOffset, dayLightSaving, leapSeconds);
        ...
        public int getTime(int debugTraceId) {
            ...
            // If "persist.radio.sib16_support" is not set, then do not send the getTime request to
            // RIL. In this case return the system time to the client
            // {@link System.currentTimeMillis()}.
            if (mSetUtcTimeProperty) {
                ret = mEmbmsOemHook.getTime(debugTraceId, msg);
            } else {
                Log.i(LOG_TAG, "persist.radio.sib16_support is not set; calling getSystemTime");
                getSystemTime(debugTraceId, msg);
            }
        ...
        public int getE911State(int debugTraceId) {
            ...
            ret = mEmbmsOemHook.getE911State(debugTraceId, msg);
        ...
        public int contentDescription(int debugTraceId, byte[] tmgi, int numberOfParameter,
            ...
            ret = mEmbmsOemHook.contentDescription(debugTraceId, mCallId, tmgi,
                    numberOfParameter, parameterCode, parameterValue, msg);
         ...
         public int getPLMNListRequest(int debugTraceId) {
            ...
            ret = mEmbmsOemHook.getPLMNListRequest(debugTraceId, msg);
