...
typedef int (ulp_quipc_process_venue_status_cb) (
   int                 quipc_error_code,
   int                 quipc_venue_info_len,
   char*               quipc_venue_info
);
...
typedef struct {
    ...
    int   (*init)( ulp_quipc_process_position_cb*         process_pos_cb,
                   ...
                   ulp_quipc_process_venue_status_cb*     venue_cb);
    ...
    /** Request Venue Enablement status */
    int   (*request_venue_status) (const UlpLocation* pos_ptr );
