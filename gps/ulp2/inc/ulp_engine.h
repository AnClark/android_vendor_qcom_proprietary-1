...
enum ulp_eng_msg_ids_t {
    ...
    // Message is sent by QUIPC provider in order to report venue status
    ULP_MSG_REPORT_QUIPC_VENUE_STATUS,
    ...
    // Message is sent by PVDService to trigger PIP venue detection
    ULP_MSG_PVD_QUERY,
...
struct ulp_msg_inject_raw_command : public ulp_msg {
        ...
        rawCommandLength(length)
    {
      ...
      if (rawCommand!=NULL)
      {
        ...
        LOC_LOGV("inject raw command of command length: %d\n ",
             rawCommandLength);
      }
      else
      {
        LOC_LOGE("Failed to allocate memory to inject command of info length %d\n ",
             rawCommandLength);
      }
    }
...
struct ulp_msg_send_pvd_query : public ulp_msg {
    const UlpPVDQueryRequest pvdQuery;
    inline ulp_msg_send_pvd_query(void* instance, UlpPVDQueryRequest request) :
        ulp_msg(instance, ULP_MSG_PVD_QUERY),
        pvdQuery(request)
    {
        LOC_LOGV("send_pvd_query flag: %d\n latitude: %f\n  longitude: %f\n  accuracy %f",
            pvdQuery.valid_flag,
            pvdQuery.latitude,
            pvdQuery.longitude,
            pvdQuery.accuracy);
...
struct ulp_msg_report_quipc_venue_status : public ulp_msg {
    const int  quipc_error_code;
    const int  rawVenueInfoLength;
    const char* rawVenueInfo;
    inline ulp_msg_report_quipc_venue_status(void* instance,
                                             int  quipc_err,
                                             int  venueInfoLength, char* venueInfo) :
        ulp_msg(instance, ULP_MSG_REPORT_QUIPC_VENUE_STATUS),
        quipc_error_code(quipc_err), rawVenueInfoLength(venueInfoLength), rawVenueInfo(new char[venueInfoLength])
    {
      if (rawVenueInfo!=NULL)
      {
        memcpy((void*)rawVenueInfo, (void*)venueInfo, venueInfoLength);

        LOC_LOGV("Quipc error code %d Inject venue info of length: %d\n ",
             quipc_error_code,
             rawVenueInfoLength);
      }
      else
      {
        LOC_LOGE("Quipc error code %d Failed to allocate memory for venue info length %d\n ",
             quipc_error_code,
             rawVenueInfoLength);
      }
    }

    inline ~ulp_msg_report_quipc_venue_status()
    {
      if (rawVenueInfo!=NULL)
      {
        delete[] rawVenueInfo;
