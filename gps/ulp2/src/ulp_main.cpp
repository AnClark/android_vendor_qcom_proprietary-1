...
static int ulp_send_pvd_query(UlpPVDQueryRequest *request);
...
static const UlpPVDQueryInterface sLocEngUlpPVDQueryInterface =
{
   sizeof(UlpPVDQueryInterface),
   ulp_send_pvd_query
};
              ...
              UlpPhoneContextCallbacks* pPhoneContextCallbacks)
{
   ...
   ulp_data.ulp_pvd_cb = pEngineCallbacks->pvd_cb;
...
const void* ulp_get_extension(const char* name)
{
   ...
   else if(strcmp(name, ULP_PVD_QUERY_INTERFACE) == 0)
   {
     ret_val = &sLocEngUlpPVDQueryInterface;
   }
...
int ulp_send_pvd_query(UlpPVDQueryRequest *request)
{
    ENTRY_LOG();
    int ret_val = 0;
    ulp_msg_send_pvd_query *msg
    (new ulp_msg_send_pvd_query(&ulp_data, *request));
