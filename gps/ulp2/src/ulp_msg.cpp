...
static int ulp_msg_process_quipc_position_report   (int   error_code,
                                                    const UlpLocation* quipc_location_ptr);
static int ulp_msg_process_quipc_venue_status      (int   error_code,
                                                    int venueInfoLength,
                                                    const char* venueInfo);
...
static int ulp_msg_process_pvd_query (const UlpPVDQueryRequest* pvdQueryPtr);
/*=====================================================================================
...
void ulp_msg_main(void * context)
{
   ...
   while (1)
   {
      ...
      // Message is sent by PVDService to query PIP venue detection in UlpPVDQueryRequest format
      case ULP_MSG_PVD_QUERY:
      {
        ulp_msg_send_pvd_query* pvdMsg = (ulp_msg_send_pvd_query*) msg;
        ulp_msg_process_pvd_query (&pvdMsg->pvdQuery);
        break;
      }
      ...
      // Message is sent by QUIPC in order to report venue status (QUIPC status)
      case ULP_MSG_REPORT_QUIPC_VENUE_STATUS:
      {
         ulp_msg_report_quipc_venue_status* statusMsg = (ulp_msg_report_quipc_venue_status*) msg;
         ulp_msg_process_quipc_venue_status (statusMsg->quipc_error_code,
                                             statusMsg->rawVenueInfoLength,
                                             statusMsg->rawVenueInfo);
         break;
      }
...
/*===========================================================================
FUNCTION    ulp_msg_process_quipc_venue_status

DESCRIPTION
   This function is called when libulp module receives a message from
   QUIPC regarding venue status.

   libulp will do necessary processing, including forwarding the appropriate
   status to PVD application(s)
...
int ulp_msg_process_quipc_venue_status(int error_code,
                                       int venueInfoLength, const char* venueInfo)
{
   int ret_val = -1;
   ...
   do
   {
      UlpPVDStatus status = ULP_PVD_STATUS_UNDETECTED;
      if (error_code == ULP_QUIPC_STATUS_NO_ERROR)
      {
         status = ULP_PVD_STATUS_DETECTED;
      }
      else if (error_code == ULP_QUIPC_STATUS_GENERAL_FAILURE)
      {
         status = ULP_PVD_STATUS_GENERAL_FAILURE;
      }

      ulp_data.ulp_pvd_cb(status, (void*)venueInfo, venueInfoLength);
      ret_val = 0;
   } while (0);
...
/*===========================================================================
FUNCTION    ulp_msg_forward_quipc_venue_status

DESCRIPTION
   This function is called when libulp module need to forward a PIP venue status
   from ULP named pipe to message queue. This is done so that all ULP
   state transition is done in one thread.
...
===========================================================================*/
int ulp_msg_forward_quipc_venue_status (int    error_code,
                                        int    venue_info_length,
                                        char*  venue_info_ptr)
{
   int ret_val = -1;
   ...
   do
   {
      ulp_msg_report_quipc_venue_status *ulpVenueStatusMsg(
            new ulp_msg_report_quipc_venue_status(&ulp_data,
                                                   error_code, venue_info_length, venue_info_ptr));
      if (ulpVenueStatusMsg == NULL)
      {
         LOC_LOGE ("%s, failed to create message: ulp_msg_report_quipc_venue_status \n", __func__);
...
/*===========================================================================
FUNCTION    ulp_msg_process_pvd_query

DESCRIPTION
   This function is called when PVDService wants to detect PIP venue

   libulp will send the request to PIP
...
===========================================================================*/
int ulp_msg_process_pvd_query (const UlpPVDQueryRequest* pvdQueryPtr)
{
   int ret_val = -1;
   UlpLocation ulpLocation;
   ...
   do
   {
      ...
      LOC_LOGD("%s flag: %d\n latitude: %f\n  longitude: %f\n  accuracy %f", __func__,
          pvdQueryPtr->valid_flag,
          pvdQueryPtr->latitude,
          pvdQueryPtr->longitude,
          pvdQueryPtr->accuracy);

      memset (&ulpLocation, 0, sizeof (UlpLocation));
      ulpLocation.gpsLocation.size = sizeof (GpsLocation);
      ulpLocation.gpsLocation.flags |= GPS_LOCATION_HAS_LAT_LONG |
                           GPS_LOCATION_HAS_ACCURACY;
      ulpLocation.gpsLocation.latitude = pvdQueryPtr->latitude;
      ulpLocation.gpsLocation.longitude = pvdQueryPtr->longitude;
      ulpLocation.gpsLocation.accuracy  = pvdQueryPtr->accuracy;
      ulpLocation.rawDataSize     = 0;
      ulpLocation.rawData         = NULL;

      if ((pvdQueryPtr->valid_flag & ULP_PVD_QUERY_HAS_POSITION) == 0)
      {
         // No position info, break
        ret_val = ulp_quipc_send_pvd_query (NULL);
      }
      else
      {
        ret_val = ulp_quipc_send_pvd_query (&ulpLocation);
      }
