   ...
   do
   {
      ...
      if(ulp_quipc_inf != NULL)
      {
         ...
         ret_val = ulp_quipc_inf->init(ulp_msg_forward_quipc_position_report,
                                       ...
                                       ulp_msg_forward_quipc_venue_status);
      }
...
/*=====================================================================================
 * Function ulp_quipc_send_pvd_query
 *
 * Description
 *  This is the function to send the PVD Query to PIP
 *
 * Parameters:
 *   UlpLocation Location where we want to detect PIP Venue
 *
 * Return value:
 *   0: SUCCESS
 *   -1: FAILURE
 =============================================================================================*/
int ulp_quipc_send_pvd_query  (const UlpLocation* coarse_pos_ptr)
{
   int ret_val = -1;
   ENTRY_LOG_CALLFLOW();
   ...
   if(ulp_quipc_inf != NULL)
   {
      LOC_LOGD ("%s: QUIPC PVD Query\n", __func__);
      ulp_quipc_inf->request_venue_status(coarse_pos_ptr);
      ret_val = 0;
   }
   else
   {
