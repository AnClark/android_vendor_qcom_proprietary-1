...
/* flag to indicate diag interface initialization */
static boolean gb_IsDiagInit = FALSE;
...
{
  slim_InitErrorEnumType e_Error = eSLIM_INIT_ERROR_FAILURE;
  if (TRUE == gb_IsDiagInit)
  {
    e_Error = eSLIM_INIT_ERROR_ALREADY_RUNNING;
  }
  else
  {
    if (TRUE == Diag_LSM_Init(NULL))
    {
      e_Error = eSLIM_INIT_SUCCESS;
      gb_IsDiagInit = TRUE;
    }
  }

  if (eSLIM_INIT_SUCCESS != e_Error)
  {
    LOC_LOGE("%s: Failed to initialize DIAG %d.", __FUNCTION__, e_Error);
  }
