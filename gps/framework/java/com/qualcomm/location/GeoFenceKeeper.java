...
public class GeoFenceKeeper {
    ...
    private void breachEvent(int engID, int whichWay, double lat, double lon, float acc) {
        Location location = new Location("IZat geofence provider");
    ...
    private void handleBreachEvent(Message m) {
        handleBreachEvent(m.arg1, m.arg2, (Location) m.obj);
    }
    private void handleBreachEvent(int id, int direction, Location location){
        ...
        if (null != geofence) {
            ...
            enteredIntent.putExtra(LocationManager.KEY_LOCATION_CHANGED, location);
            try {
