...
const bool LBSProxy::mComQualcommLocationApkExists =
    LBSProxy::checkIfComQualcommLocationApkExists();

LocApiBase* LBSProxy::getLocApi(const MsgTask* msgTask,
                                LOC_API_ADAPTER_EVENT_MASK_T exMask,
                                ContextBase *context) const {
#ifdef _HAS_LOC_V02_
    if (loc_get_target() == TARGET_PDS)
    {
        void *handle = NULL;
        if((handle = dlopen("libizat_api_pds.so", RTLD_NOW)) != NULL) {
            getLocApi_t* getter = (getLocApi_t*)dlsym(handle, "getLocApi");
            if(getter != NULL) {
                return (*getter)(msgTask, exMask, context);
            }
        }
    }
...
bool LBSProxy::hasAgpsExtendedCapabilities() const {
    ...
    return mComQualcommLocationApkExists;
}
...
bool LBSProxy::checkIfComQualcommLocationApkExists()
{
