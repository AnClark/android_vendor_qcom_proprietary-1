...
static void breachEventCb(int engID, GpsLocation* location, int32_t transition, GpsUtcTime timestamp) {
    ...
    env->CallVoidMethod(mGeoFenceKeeper, method_breachEvent,
                        engID, whichWay, (jdouble)location->latitude,
                        (jdouble)location->longitude, (jfloat)location->accuracy);
...
{
    method_breachEvent = env->GetMethodID(clazz, "breachEvent", "(IIDDF)V");
