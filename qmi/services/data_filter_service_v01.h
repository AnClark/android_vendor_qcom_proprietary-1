...
#define DFS_V01_IDL_MINOR_VERS 0x06
/** Major Version Number of the qmi_idl_compiler used to generate this file */
#define DFS_V01_IDL_TOOL_VERS 0x06
/** Maximum Defined Message ID */
#define DFS_V01_MAX_MESSAGE_ID 0x0031
/**
    @}
  */
...
typedef struct {
  ...
  */
  ...
  /**<    Maximum number of filters that can be added for low latency in total
        and per filter add request
  */
...
typedef enum {
  ...
  DFS_LOW_LATENCY_FILTER_DIRECTION_DOWNLINK_V01 = 0x00, /**<  Low latency traffic detection filter to be installed in Downlink
       data direction  */
  DFS_LOW_LATENCY_FILTER_DIRECTION_UPLINK_V01 = 0x01, /**<  Low latency traffic detection filter to be installed in Uplink
       data direction  */
...
typedef struct {
  ...
  /**<   Inter arrival time in ms between successive packets for each
       filter rule. This will determine the duration of the timer that is run
  ...
  /**<   Data traffic direction in which the filter rules in the filter_rule
 TLV need to be installed. Default value is
 DFS_LOW_LATENCY_FILTER_DIRECTION_UPLINK. Values : \n
      - DFS_LOW_LATENCY_FILTER_DIRECTION_DOWNLINK (0x00) --  Low latency traffic detection filter to be installed in Downlink
       data direction
      - DFS_LOW_LATENCY_FILTER_DIRECTION_UPLINK (0x01) --  Low latency traffic detection filter to be installed in Uplink
       data direction
 */
...
typedef struct {
  ...
  uint32_t filter_handles_len;  /**< Must be set to # of elements in filter_handles */
  uint32_t filter_handles[QMI_DFS_MAX_FILTERS_V01];
  /**<   List of handles to the filter rules that that have been added.
       This TLV will be returned when there is no error
   */
  ...
  dfs_filter_param_error_mask_v01 filter_rule_error[QMI_DFS_MAX_FILTERS_V01];
  /**<   List of error masks that contain errors for each filter rule. This TLV
       will be returned when the error is QMI_ERR_INVALID_ARG.
   */
...
/** @addtogroup dfs_qmi_messages
    @{
  */
/** Indication Message; Indication sent to the TE from the device when it detects the start
           or end of low latency traffic */
typedef struct {
...
typedef enum {
  ...
  DFS_REVERSE_IP_TRANSPORT_FILTERS_ADDED_V01 = 0, /**<  Reverse IP Transport filters have been added  */
  DFS_REVERSE_IP_TRANSPORT_FILTERS_DELETED_V01 = 1, /**<  Reverse IP Transport filters have been deleted  */
...
/** @addtogroup dfs_qmi_messages
    @{
  */
/** Indication Message; Indication sent to the TE from the device that contains change in
           reverse IP transport filters */
typedef struct {

  /* Mandatory */
  /*  Filter Action */
  dfs_reverse_ip_transport_filters_action_enum_v01 filter_action;
  /**<   Indicates whether the filters need to be added or deleted. Values : \n
      - DFS_REVERSE_IP_TRANSPORT_FILTERS_ADDED (0) --  Reverse IP Transport filters have been added
      - DFS_REVERSE_IP_TRANSPORT_FILTERS_DELETED (1) --  Reverse IP Transport filters have been deleted
 */
