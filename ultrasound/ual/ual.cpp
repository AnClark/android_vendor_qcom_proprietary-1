...
(
  ...
  // Callback for Rx state changes (audio device arbitration)
...
)
{
  ...
  if (rx_state_evt_cb != NULL)
  {
    if (ual_aud_dev_arbi_init(rx_state_evt_cb) < 0)
    {
    ...
    if (ual_aud_dev_arbi_acquire(AUDIO_DEVICE_OUT_EARPIECE) < 0)
    {
      LOGE("%s:  Cannot acquire device",
        __FUNCTION__);
      (void)pthread_mutex_unlock(&s_ual_RX_control_mutex);
      return false;
    }
  ...
  bool notify_device_release)
{
  ...
  {
    if (ual_aud_dev_arbi_release(AUDIO_DEVICE_OUT_EARPIECE) < 0)
    {
      LOGE("%s:  Error notifying media server that device is released",
           __FUNCTION__);
    }
  }
