...
#else
#define FLASHLIGHT0_SYS_PATH "/sys/class/leds/torch-light0/brightness"
...
void enable(bool enable) {
    ...
    if(enable) {
        ...
        snprintf(str, sizeof(str), "%s: %s", mmi_lang::query("flashlight"), mmi_lang::query("on"));
        btn->set_text(str);
    } else {
        write_file(FLASHLIGHT0_SYS_PATH, "0");
        write_file(FLASHLIGHT1_SYS_PATH, "0");
        snprintf(str, sizeof(str), "%s: %s", mmi_lang::query("flashlight"), mmi_lang::query("off"));
        btn->set_text(str);
    }
}
...
void get_input() {
    ...
    ALOGE("config Flashlight ,timeout=%d(ms), exit_state=%d", input.timeout, input.exit_state);
}
...
int auto_test() {
    ...
    get_input();
    if(!strncmp(g_module->get_domain(), "FLASHLIGHT_ON", strlen("FLASHLIGHT_ON"))) {
        if(input.exit_state == true) {
            ret |= enableNoUi(true);
        } else {
            return CASE_FAIL;
        }
    } else if(!strncmp(g_module->get_domain(), "FLASHLIGHT_OFF", strlen("FLASHLIGHT_OFF"))) {
