...
void show_message(int cardnum, bool forall, const char *str) {
    ...
    if(forall) {
        for(i = 0; i < input.sim_num; i++) {
            snprintf(temp, sizeof(temp), "%s%d: %s", mmi_lang::query("simcard"), i, str);
            btn_sim[i]->set_text(temp);
        }
    } else {
        if(cardnum < input.sim_num) {
            snprintf(temp, sizeof(temp), "%s%d: %s", mmi_lang::query("simcard"), cardnum, str);
...
void *processThread(void *) {
    ...
    char temp[256] = { 0 };
    show_message(0, true, mmi_lang::query("detecting"));
    exe_sim_command();
    get_sim_state();
    for(int i = 0; i < input.sim_num; i++) {
        if(sim_state[i])
            strlcpy(str, mmi_lang::query("present"), sizeof(str));
        else
            strlcpy(str, mmi_lang::query("absent"), sizeof(str));
