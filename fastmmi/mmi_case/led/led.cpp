...
void enable(bool on) {
    ...
    switch (input.color) {
    case LED_RED:
        snprintf(temp, sizeof(temp), "%s: %s", mmi_lang::query("led_red"),
                 on ? mmi_lang::query("on") : mmi_lang::query("off"));
        break;
    case LED_BLUE:
        snprintf(temp, sizeof(temp), "%s: %s", mmi_lang::query("led_blue"),
                 on ? mmi_lang::query("on") : mmi_lang::query("off"));
        break;
    case LED_GREEN:
        snprintf(temp, sizeof(temp), "%s: %s", mmi_lang::query("led_green"),
                 on ? mmi_lang::query("on") : mmi_lang::query("off"));
        break;
    default:
        break;
...
void get_input() {
    ...
    ALOGE("config LED , %s:%d. path=%s; timeout=%d(ms), exit_state:%d",
          input.color_name, input.color, input.path, input.timeout, input.exit_state);
       ...
       !strncmp(g_module->get_domain(), "LED_GREEN_ON", strlen("LED_GREEN_ON"))) {
        if(input.exit_state == true) {
            ...
            return CASE_FAIL;
                       ...
                       "LED_RED_OFF",
                       strlen("LED_RED_OFF")) ||
              !strncmp(g_module->get_domain(), "LED_GREEN_OFF", strlen("LED_GREEN_OFF"))) {
