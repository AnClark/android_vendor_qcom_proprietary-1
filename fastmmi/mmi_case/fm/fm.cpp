...
void initUI() {
    ...
    btn_next = new mmi_button(rect, mmi_lang::query("btn_search_next"), next_tune_cb);
...
static int fm_seeknext() {
    ...
    ALOGE("cmd:%s \n", fm_test_cmd);
    btn_fm->set_text(mmi_lang::query("search_tune"));
    invalidate();
    local_exe_cmd(fm_test_cmd, result, sizeof(result));
    ALOGE("fm_seeknext result:%s \n", result);
    tune = atoi(result);
    snprintf(temp, sizeof(temp), "%s:%d.%dMHz", mmi_lang::query("playing_tune"), tune / 1000, (tune % 1000) / 100);
    btn_fm->set_text(temp);
    invalidate();
    sleep(2);
    return 0;
...
static int fm_config_audio() {
    ...
    btn_fm->set_text(mmi_lang::query("config_audio"));
    invalidate();
    exe_set_audio_path();
