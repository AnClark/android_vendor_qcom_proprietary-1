...
void get_string(char *str, struct mm_audio_ftm_params *params, size_t size) {
    ...
    case TC_PCM_REC_HANDSET_MIC:
        snprintf(str, size, "%s %d %s", mmi_lang::query("handset_record"), params->duration,
                 mmi_lang::query("seconds"));
        break;
    case TC_PCM_PLAY_HANDSET_EARPHONE:
        snprintf(str, size, "%s", mmi_lang::query("handset_playback"));
        break;
    case TC_PCM_REC_HEADSET_MIC:
        snprintf(str, size, "%s %d %s", mmi_lang::query("headset_record"), params->duration,
                 mmi_lang::query("seconds"));
        break;
    case TC_PCM_PLAY_HEADSET_STEREO_LR:
        snprintf(str, size, "%s", mmi_lang::query("headset_playback"));
        break;
    case TC_PCM_PLAY_HANDSET_LOUDSPEAKER:
        snprintf(str, size, "%s", mmi_lang::query("loudspeaker_playback"));
        break;
    case TC_PCM_REC_HANDSET_SECOND_MIC:
        snprintf(str, size, "%s %d %s", mmi_lang::query("headset_second_record"), params->duration,
                 mmi_lang::query("seconds"));
        break;
    default:
        break;
...
void *processThread(void *) {
    ...
    btn_show->set_text(mmi_lang::query("finish"));
...
int auto_test() {
    ...
    if(rec_params.testcase != TC_PCM_REC_HEADSET_MIC || play_params.testcase != TC_PCM_PLAY_HEADSET_STEREO_LR) {
        return CASE_FAIL;
    }
