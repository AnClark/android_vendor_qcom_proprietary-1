...
void enable(bool enable) {
    ...
    if(enable) {
        ...
        snprintf(str, sizeof(str), "%s: %s", mmi_lang::query("keypad_backlight"), mmi_lang::query("on"));
        btn->set_text(str);
    } else {
        write_file(input.path, "0");
        snprintf(str, sizeof(str), "%s: %s", mmi_lang::query("keypad_backlight"), mmi_lang::query("off"));
        btn->set_text(str);
    }
}
