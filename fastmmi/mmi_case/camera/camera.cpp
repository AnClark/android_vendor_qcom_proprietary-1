...
typedef int32_t(*mm_app_stream_clean_invalidate_buf_lib_t) (uint32_t index, void *user_data);
typedef int32_t(*mm_app_stream_invalidate_buf_lib_t) (uint32_t index, void *user_data);
...
typedef int (*mm_app_set_preview_fps_range_t) (mm_camera_test_obj_t *test_obj, cam_fps_range_t *fpsRange);
...
mm_app_set_preview_fps_range_t mm_app_set_preview_fps_range_lib;
...
void subMenu() {
    ...
    rect.w = width / 3;
    rect.h = height / 8;
    btn = new mmi_button(rect, mmi_lang::query("pass"), btn_cb_pass);
    btn->set_color(0, 125, 125, 255);
    g_module->add_btn(btn);

    rect.x = width / 3;
    btn = new mmi_button(rect, mmi_lang::query("fail"), btn_cb_fail);
    btn->set_color(125, 125, 0, 255);
    g_module->add_btn(btn);

    rect.x = width * 2 / 3;
    btn = new mmi_button(rect, mmi_lang::query("snapshot"), btn_cb_takepicture);
...
int init_camera() {
    ...
    mm_app_set_preview_fps_range_lib =
      (mm_app_set_preview_fps_range_t) dlsym(libqcamera, "mm_app_set_preview_fps_range");
    if (!mm_app_set_preview_fps_range_lib) {
      ALOGE("FATAL ERROR: could not dlsym mm_app_set_preview_fps_range\n");
    }
    /*load hal interface*/
    ...
    if (mm_app_set_preview_fps_range_lib != NULL) {
      mm_app_set_preview_fps_range_lib(&camera_obj, &preview_fps_range);
    }

    /*set EV*/
    if (mm_app_set_params_lib != NULL) {
      mm_app_set_params_lib(&camera_obj, CAM_INTF_PARM_AEC_ALGO_TYPE, 0);
      mm_app_set_params_lib(&camera_obj, CAM_INTF_PARM_ANTIBANDING, CAM_ANTIBANDING_MODE_AUTO);
    }
