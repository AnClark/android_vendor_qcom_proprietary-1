...
int input_callback(int fd, unsigned int revents, void *data) {
...
int get_ts_resolution() {
    ...
    for(i = 0; i < max_event_index + 1; i++) {
        ...
        for(j = 0; j < BITS_TO_LONGS(ABS_MAX); j++)
            ALOGI("%s: absBitmask: %ld (%d,%d)\n", filepath, absBitmask[j], BITS_PER_LONG, ABS_MT_POSITION_X);
        ...
        if(test_bit(ABS_MT_POSITION_X, absBitmask)
           && test_bit(ABS_MT_POSITION_Y, absBitmask)) {
            ...
            if(ioctl(fd, EVIOCGABS(ABS_MT_POSITION_X), state_instance->get_ts_abs_X()) < 0 ||
               ioctl(fd, EVIOCGABS(ABS_MT_POSITION_Y), state_instance->get_ts_abs_Y()) < 0) {
