...
void btn_cb_shutdown(void *component) {
    mmi_button *btn = (mmi_button *) component;

    btn->set_text("Shutdown...");
    system("reboot -p");
    exit(0);
}

static void reset_usb_config() {
    /*set usb config to NULL , system will boot up to default settings */
...
void load_one_test_case(const char *filename, const char *domain) {
    ...
    snprintf(lib_file, sizeof(lib_file), "%s/%s", MMI_LIB_PATH, filename);
...
void build_main_ui() {
    ...
    for(iter = module_list->begin(), i = 0; iter != module_list->end(); iter++, i++) {
        ...
        key = mmi_config::query_config_value(mod->get_domain(), "display_name", mod->get_domain());
        snprintf(text, sizeof(text), "%s", mmi_lang::query(key));
    ...
    rect.w = dft_rect_w();
    rect.h = dft_rect_w();
    class mmi_button *btn = new mmi_button(rect, mmi_lang::query("page_up"), btn_cb_up);

    btn->set_color(0, 0, 0, 0);
    get_main_module()->add_btn(btn);

    rect.y = gr_fb_height() / 2;
    btn = new mmi_button(rect, mmi_lang::query("page_down"), btn_cb_down);
    btn->set_color(0, 0, 0, 0);
    get_main_module()->add_btn(btn);

    rect.y = gr_fb_height() * 3 / 4;
    btn = new mmi_button(rect, mmi_lang::query("shut_down"), btn_cb_shutdown);
    btn->set_color(0, 0, 0, 0);
    get_main_module()->add_btn(btn);

    rect.y = gr_fb_height() - dft_rect_w();
    btn = new mmi_button(rect, mmi_lang::query("finish"), btn_cb_finish);
    btn->set_color(0, 0, 0, 0);
    get_main_module()->add_btn(btn);
    invalidate();
}
...
static int set_all_modules_status(void) {
    char line[1024] = { 0, };
    ...
    FILE *file = fopen(state_instance->get_cur_result_file(), "r");

    if(file == NULL) {
        ALOGE("Cannot open result file %s", state_instance->get_cur_result_file());
        return -1;
    }
    ...
    while(fgets(line, sizeof(line), file) != NULL) {
        ...
        if(strncmp(line, "Result", 6) == 0) {
            ...
            strlcpy(result, line + splitIndex + 2, 5);
            if(strncmp(result, "Pass", 4) == 0)
                res = 0;
            else if(strncmp(result, "Fail", 4) == 0)
                res = -1;
            else
                res = 1;

            mmi_module *module = get_module_by_domain(cur_domain);

            if(module != NULL)
                module->set_result(res);
...
int exit_single_case(bool is_pass) {

    int x = CENTER_POINT_PASS_X;

    if(!is_main_module(get_current_module())) {
        ALOGE("current main module is running, so exit \n");
        return FTM_FAIL;
    }

    if(!is_pass)
        x = CENTER_POINT_FAIL_X;

    screen_pressed(x, RECT_Y);
    return FTM_SUCCESS;
}
...
static void init_language(struct input_params *input) {

    /*initial multi-language instance */
    if(!strncmp(input->language, "zh-rCN", 6))
        mmi_lang::load(ZH_rCN);
    else
        mmi_lang::load(EN);

}
void init_config(struct input_params *input) {
    ...
    strlcpy(input->misc_dev, mmi_config::query_config_value(MMI_DOMAIN, "misc_dev", DEFAULT_MISC_DEV_PATH),
            sizeof(input->misc_dev));
    strlcpy(input->sys_backlight, mmi_config::query_config_value(MMI_DOMAIN, "sys_backlight", DEFAULT_MISC_DEV_PATH),
            sizeof(input->sys_backlight));
    strlcpy(input->language, mmi_config::query_config_value(MMI_DOMAIN, "language", DEFAULT_LANGUAGE),
            sizeof(input->language));
    ...
    if(mode != NULL) {
        for(i = 0; i < MAX_TEST_MODE; i++) {
            if(!strncmp(test_mode_string[i], mode, strlen(test_mode_string[i]))) {
                input->test_mode = (case_run_mode_t) i;
                found = true;
                break;
            }
        }
    ...
    if(!found)
        input->test_mode = TEST_MODE_NONE;

    ALOGE("mmi input: misc_dev:%s;  backlight:%s; test_mode=%s\n", mmi_input.misc_dev,
          mmi_input.sys_backlight, test_mode_string[mmi_input.test_mode]);
}
