...
void mmi_module::add_btn_pass(mmi_cb_t cb) {
    mmi_rect_t rect = { RECT_PASS_X, RECT_Y, RECT_WIDTH, RECT_HEIGHT };
    class mmi_button *btn = new mmi_button(rect, mmi_lang::query("pass"), cb);

    btn->set_color(0, 125, 125, 255);
    btn->set_launch_module(this);
    this->add_btn(btn);
}

void mmi_module::add_btn_fail(mmi_cb_t cb) {
    mmi_rect_t rect = { RECT_FAIL_X, RECT_Y, RECT_WIDTH, RECT_HEIGHT };
    class mmi_button *btn = new mmi_button(rect, mmi_lang::query("fail"), cb);
...
int mmi_module::mod_sem_timewait(const struct timespec *ts) {
    int s;
    ...
    if(s == -1) {
        if(errno == ETIMEDOUT)
            ALOGE("sem_timedwait() timed out\n");
        else
            ALOGE("sem_timedwait");
    } else
        ALOGI("sem_timedwait() succeeded\n");
    return ((s == 0) ? 0 : -1);
}
