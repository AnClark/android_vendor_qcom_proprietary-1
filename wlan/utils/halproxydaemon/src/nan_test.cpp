...
namespace NAN_TEST
{
        ...
        :wifiHandle_(handle)
    {
        ...
        fprintf(stderr, "%s: Passing UserData 0x%x\n", __func__, this);
        nan_register_handler(wifiHandle_, callbackHandler, this);
        gExitflag = 0;
    }
    ...
    void NanTestSuite::nanEventPublishTerminated(NanPublishTerminatedInd *event,
                                                 void *userdata)
    {
        fprintf(stderr, "%s: handle %d reason %d userdata 0x%x\n",
          __FUNCTION__,
         event->header.handle,
         event->reason,
         userdata
                                     ...
                                     void *userdata)
    {
        fprintf(stderr, "%s: handle %d match_handle %08x " MAC_ADDR_STR " rssi:%d "
                "Match Occured Flag:%d Out of Resource Flag:%d userdata 0x%x\n",
          __FUNCTION__,
          event->header.handle,
          event->match_handle,
          MAC_ADDR_ARRAY(event->addr),
    ...
    void NanTestSuite::nanEventUnMatch(NanUnmatchInd *event,
                                       void *userdata)
    {
        fprintf(stderr, "%s: handle %d match_handle %08x userdata 0x%x\n",
          __FUNCTION__,
          event->header.handle,
          event->match_handle,
          userdata
    ...
    void NanTestSuite::nanEventDiscEngEvent(NanDiscEngEventInd *event,
                                            void *userdata)
    {
        fprintf(stderr, "%s: handle %d event_id %d userdata 0x%x\n",
          __FUNCTION__,
         event->header.handle,
         event->event_id,
         userdata
                                   ...
                                   void *userdata)
    {
        fprintf(stderr, "%s: handle %d tca_id %d "
                "threshold risen %d threshold fallen %d userdata 0x%x\n",
          __FUNCTION__,
         event->header.handle,
         event->tca_id,
                                          ...
                                          void *userdata)
    {
        fprintf(stderr, "%s: handle %d addr " MAC_ADDR_STR
                "isVsa %d isBeaconSdfPayload %d userdata 0x%x\n",
          __FUNCTION__,
         event->header.handle,
         MAC_ADDR_ARRAY(event->addr),
