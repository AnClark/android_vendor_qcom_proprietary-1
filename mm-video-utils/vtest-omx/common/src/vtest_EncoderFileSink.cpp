...
EncoderFileSink::EncoderFileSink(Crypto *pCrypto)
    : ISource(),
      ...
      m_Scale(0),
      m_Rate(0),
      m_FrameCount(0), //VPX frame counter
      m_bSecureSession(OMX_FALSE),
      m_pOutCopyBuf(NULL),
      m_nOutCopyBufSize(0),
      m_pCrypto(pCrypto) {

    sprintf(m_pName, "EncoderFileSink");
...
OMX_ERRORTYPE EncoderFileSink::Configure(CodecConfigType *pConfig,
    ...
    m_Width = pConfig->nFrameWidth;
    m_Height = pConfig->nFrameHeight;
    ...
    if (pConfig->eCodec == OMX_VIDEO_CodingVPX) {
        m_bIsVPX = OMX_TRUE;
        m_Scale = 1; // Microsecond timebase for OMX
        m_Rate = 1000000;
    }
...
OMX_ERRORTYPE EncoderFileSink::ThreadRun(OMX_PTR pThreadData) {
    ...
    if (m_bIsVPX && !m_bSecureSession) {
        writeFrame(makeHeaderIvf(
            m_FrameCount, m_Width, m_Height, m_Scale, m_Rate));
    }
    ...
    for (OMX_S32 i = 1; !m_bThreadStop; i++) {
        ...
        if (result != OMX_ErrorNone || pBuffer == NULL) {
    ...
    if (m_bSecureSession) {
        ...
        if (m_nOutCopyBufSize >= pHeader->nFilledLen) {
            ...
            if (m_pCrypto != NULL && pPmem != NULL && pPmem->fd > 0) {
                m_pCrypto->Copy(SAMPLECLIENT_COPY_SECURE_TO_NONSECURE,
                        m_pOutCopyBuf, pPmem->fd, pHeader->nFilledLen);
                result = m_pFile->Write(m_pOutCopyBuf,
        ...
        switch ((OMX_U32)pExtra->eType) {
            case OMX_ExtraDataVideoEncoderMBInfo:
            {
                ...
                if (mbi->nFormat == 1) {
                    ...
                    height_mbs = (m_Height + 15) / 16;
                    width_mbs = (m_Width + 15) / 16;
