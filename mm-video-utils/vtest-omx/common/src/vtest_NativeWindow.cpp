...
NativeWindow::NativeWindow(OMX_BOOL bRotate)
    : ISource(),
      ...
      m_nDisplayHeight(0),
      m_nMinBufferCount(0),
      m_eColorFormat((OMX_COLOR_FORMATTYPE)QOMX_COLOR_FORMATYUV420PackedSemiPlanar32m),
...
PortBufferCapability NativeWindow::GetBufferRequirements(OMX_U32 ePortIndex) {
    ...
    sBufCap.nMinBufferCount = (m_pSink != NULL) ?
        MAX(m_pSource->GetBufferRequirements(PORT_INDEX_OUT).nMinBufferCount,
                m_pSink->GetBufferRequirements(PORT_INDEX_IN).nMinBufferCount)
        : m_nMinBufferCount;
...
OMX_ERRORTYPE NativeWindow::Start() {
    ...
    OMX_U32 nBufferCount;
    {
        ...
        result = m_pBufferManager->GetBuffers(this,
                PORT_INDEX_IN, &pBuffers, &nBufferCount);
        FAILED1(result, "Error setting port buffers");

        for (OMX_U32 i = 0; i < mBufMap->GetSize(); ++i) {
            result = RegisterBuffer(
                    (OMX_U8 *)pBuffers[i].pHandle, pBuffers[i].pHeaderIn);
            FAILED1(result, "Could not register buffer : %p, handle : 0x%lu",
                    pBuffers[i].pHeaderIn, pBuffers[i].pHandle);
        }
...
OMX_ERRORTYPE NativeWindow::Configure(CodecConfigType *pConfig,
    ...
    m_nMinBufferCount = (OMX_U32)minUndequeuedBufs + 1;
    return result;
}
...
OMX_ERRORTYPE NativeWindow::SetBuffer(
    ...
    if (pSource == m_pSource) {
        ...
        FAILED2(result, m_bThreadStop, "Error in finding buffer %p", pBuffer);
...
OMX_ERRORTYPE NativeWindow::AllocateBuffers(BufferInfo **pBuffers,
    ...
    VTEST_MSG_MEDIUM("nativeWindow set geometry: w=%u h=%u colorFormat=%x",
            (unsigned int)m_nFrameWidth, (unsigned int)m_nFrameHeight, m_eColorFormat);
    err = native_window_set_buffers_geometry(m_pNativeWindow.get(),
            m_nFrameWidth, m_nFrameHeight, m_eColorFormat);
    if (err != OK) {
...
OMX_ERRORTYPE NativeWindow::ThreadRun(OMX_PTR pThreadData) {
    ...
    for (OMX_U32 i = 1; m_bThreadStop != OMX_TRUE; i++) {
        ...
        if ((pFreeHeader == NULL) && (m_nBuffersWithNativeWindow > m_nMinBufferCount)) {
            result = DequeueBuffer(pFreeHeader);
            if (result != OMX_ErrorNone) {
                VTEST_MSG_ERROR("Error while dequing buffer");
                m_bThreadStop = OMX_TRUE;
            }
            m_nBuffersWithNativeWindow--;
        }
...
OMX_ERRORTYPE NativeWindow::CancelBuffer(OMX_BUFFERHEADERTYPE *pHeader) {
    ...
    if (buf != NULL) {
        ...
        VTEST_MSG_ERROR("Could not find a buffer to cancel, buffers need to be registered before!");
        result = OMX_ErrorUndefined;
    }
    return result;
}
...
OMX_ERRORTYPE NativeWindow::EnqueueBuffer(OMX_BUFFERHEADERTYPE *pHeader) {
    ...
    if (buf == NULL) {
        ...
        buf = mBufMap->find(pHeader);
        if ((result != OMX_ErrorNone) || (buf == NULL)) {
...
void NativeWindow::fixScaling(OMX_U32 transform,
    ...
    OMX_U32 dstW = (transform == 0) ? m_nDisplayWidth : m_nDisplayHeight;
    OMX_U32 dstH = (transform == 0) ? m_nDisplayHeight : m_nDisplayWidth;

    OMX_U32 scaleX_x1000 = (dstW * 1000) / m_nFrameWidth;
    OMX_U32 scaleY_x1000 = (dstH * 1000) / m_nFrameHeight;

    //maintain the aspect ratio
    OMX_U32 scale_x1000 = (scaleX_x1000 < scaleY_x1000) ? scaleX_x1000 : scaleY_x1000;
