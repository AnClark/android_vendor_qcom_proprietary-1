...
uint64 MP2StreamParser::GetClipDurationInMsec()
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        if(m_pStream_Info[i].stream_media_type == TRACK_TYPE_AUDIO &&
           bPlayAudio && m_nLastAudioPTS)
        {
          audioDuration = (uint64)(m_nLastAudioPTS - m_nRefAudioPTS);
        }
        else if (m_pStream_Info[i].stream_media_type == TRACK_TYPE_VIDEO &&
                 bPlayVideo && m_nLastVideoPTS)
        {
...
MP2StreamStatus MP2StreamParser::StartParsing(void)
{
  ...
  {
    bool bResetToStart = true;
    while(bContinue                     &&
         (m_nCurrOffset < m_nFileSize)  &&
         (m_pDataBuffer)                &&
         (!m_bProgramStream))
    {
      //! After PMT is found, reset offset to ZERO.
      //! Sync frame may available before PAT/PMT.
      if(m_ProgMapSection && bResetToStart)
      {
        m_nCurrOffset  = 0;
        m_nStartOffset = 0;
        bResetToStart  = false;
      ...
      if(bContinue)
      {
        ...
        if((m_bGetLastPTS) && (m_bInitialParsingPending))
        {
          ...
          if((counter > MAX_TS_PKTS_FOR_DURATION) &&
             (m_nLastVideoPTS  || m_nLastAudioPTS))
          {
            if(m_nLastVideoPTS)
        ...
        if(MP2STREAM_SUCCESS != retError)
        {
          ...
          MM_MSG_PRIO3(MM_FILE_OPS, MM_PRIO_ERROR,
               "StartParsing: Read failed @ %llu, lastPTS %d, parser state %d",
               m_nCurrOffset, m_bGetLastPTS, m_eParserState);
          if (m_bGetLastPTS)
          {
            m_nLastAudioPTS = m_nRefAudioPTS;
            m_nLastVideoPTS = m_nRefVideoPTS;
          }
          m_bInitialParsingPending = false;
          m_bGetLastPTS = false;
          bContinue = false;
        ...
        #endif
        // Do not prepare Codec Config data, if client does not request
        //Currently we will make only for H.264 (look for codec info in AUs)
        if( (bContinue) && (m_bInitialParsingPending) &&
            (!m_bGetLastPTS) && (m_bLocateCodecHdr))
        {
          uint8 newPayload = 0;
          uint32 frameLen  = 0;
          uint32 trackId   = 0;
          ...
          if(m_pVideoStreamIds)
          {
            trackId = (uint32)m_pVideoStreamIds[0];
          }
          MP2StreamStatus eRet = GetTrackType(trackId,&ttype,&medtype);

          //! Check for codec config data only if Codec is H264
          if((MP2STREAM_SUCCESS == eRet) &&
             (NULL == m_pAvcCodecInfo || false == m_pAvcCodecInfo->isValid) &&
             ((TRACK_TYPE_VIDEO == ttype) && (medtype == VIDEO_CODEC_H264)) )
          {
            uint64 ullStartOffset = 0;
            //! Update offset value only if Video ID matches with TS Packet ID
            if((m_currTSPkt.pyld_unit_start_indicator) &&
               (trackId == m_currTSPkt.PID))
            {
              if(m_bIsBDMVFormat)
                ullStartOffset = m_nCurrOffset - MPEG2_M2TS_PKT_BYTES;
              else
                ullStartOffset = m_nCurrOffset - TS_PKT_SIZE;
            }
            if(isSameStream(&trackId,&newPayload))
            {
                ...
                if((!m_pAvcCodecInfo) || (!m_pAvcCodecInfo->isValid))
                {
                  ...
                  if(bRet)
                  {
                    ...
                    if(bret)
                    {
                      ...
                      //! Update start offset to first IDR frame
                      m_nStartOffset = ullStartOffset;
                    }
                  }
                  else
                  {
