  ...
  while(ulChunkCount*MPEG2_FILE_READ_CHUNK_SIZE < m_nFileSize)
  {
    ...
    else
    {
      ...
      if(MP2STREAM_SUCCESS == retError)
      {
        ...
        if(!readMpeg2StreamData (ullAbsOffset, 2*sizeof(MP2_SYS_HDR_START_CODE),
        ...
        if(bContinue)
        {
          ...
          if( codeVal == MP2_SYS_HDR_START_CODE)
          {
            ...
            //! Do not parse System target header during duration calc.
              ...
              TS_PES_PKT_TOTAL_HEADER_SIZE))
      {
        if( (m_currPESPkt.pts_dts_flag & 0x03) == 0x03)
        {
             ...
             (m_currPESPkt.trackid == m_nAudioPIDSelected))
          {
            ...
            {
              m_nLastAudioPTS = m_currPESPkt.pts;
            }
                  ...
                  (m_currPESPkt.trackid == m_nVideoPIDSelected))
          {
            ...
            {
              m_nLastVideoPTS = m_currPESPkt.pts;
            }
            if(!m_bInitialParsingPending)//During playback
            {
              /* Correct if discontinuity present */
...
bool MP2StreamParser::parseAC3Header(audio_info* audioInfo)
{
  ...
  if( (frmsizecod >= 0x00) && (frmsizecod <= 0x25) )
  {
    bit_rate = FRAME_SIZE_CODE_TABLE[frmsizecod].nominal_bit_rate * 1000;
    MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_MEDIUM,"AC3 bit-rate %lu",bit_rate);
  }
...
@note      None.
========================================================================== */
bool  MP2StreamParser::GetAVCCodecInfo(uint32* pulBufSize, uint8* pucDataBuf)
{
  uint32 ulNALSize       = 0;
  uint32 ulBufSize       = 0;
  uint32 ulAVCIndex      = 0;
  ...
  if(m_pAvcCodecInfo)
  {
    while((ulBufIndex < *pulBufSize))
    {
      ...
      bRet = GetNextH264NALUnit(ulBufIndex, pucDataBuf, &ucNALUType,
                                &ulNALSize, *pulBufSize, &ulNALStartIndex);
      if(bRet)
      {
        bRet = false;
        ...
        {
          uint8* pCodecBuf = m_pAvcCodecInfo->codecInfoBuf;
          if(NAL_UNIT_TYPE_SPS == ucNALUType)
            bSPSFound = true;
          else
            bPPSFound = true;
          ...
          if(NULL == pCodecBuf)
          {
            ...
            MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_LOW,
                         "GetAVCCodecInfo allocating %lu", (ulBufSize));
          }
          //! If size allocated is less than required
          else if(m_pAvcCodecInfo->size < (ulAVCIndex + ulNALSize))
          {
            ulBufSize = (ulBufSize + ulNALSize) * 2;
            pCodecBuf = (uint8*)MM_Realloc(pCodecBuf, ulBufSize);
            MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_LOW,
                         "GetAVCCodecInfo realloc %lu", ulBufSize);
          }
          if(pCodecBuf)
          {
            m_pAvcCodecInfo->codecInfoBuf = pCodecBuf;
            m_pAvcCodecInfo->size = ulBufSize;
            memcpy(m_pAvcCodecInfo->codecInfoBuf + ulAVCIndex,
          ...
          else
          {
            ...
            m_pAvcCodecInfo->codecInfoBuf = NULL;
            m_pAvcCodecInfo->size = ulAVCIndex = 0;
            MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_ERROR,
    ...
    bRet = false;
    if (m_pAvcCodecInfo->isValid)
    {
      bRet = true;
      //! update size to indicate amount of data filled in buffer
      m_pAvcCodecInfo->size = ulAVCIndex;
    }
  } // if(m_pAvcCodecInfo)
