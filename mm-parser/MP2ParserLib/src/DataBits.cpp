    ...
    if(m_bHttpStreaming)
    {
      ...
      else
      {
        MM_MSG_PRIO3(MM_FILE_OPS, MM_PRIO_HIGH,
            ...
            ullOffset,nNumBytesRequest, availOffset);
        //! It is a special case to handle read failures in streaming mode.
        //! Parser will treat this as non-recoverable error and exit playback
        //! immediately. Below hack is to ensure the exit playback
        if((FILE_SOURCE_MP2TS == m_eFileFormat) &&
           (-1 != m_nFileSize))
        {
