        ...
        if(albStatus==FILE_SOURCE_SUCCESS && nMetaBuff > 0 ) {
            ...
            if(bIsUTF8) {
                ...
                if(buffer != NULL && len > 0) {
                    utf32_to_utf8((char32_t *)albBuffer, len, buffer, bytes);
        ...
        if (status == FILE_SOURCE_SUCCESS){
            ...
            if (majorType == FILE_SOURCE_MJ_TYPE_AUDIO){
                ...
                if((status==FILE_SOURCE_SUCCESS) && nFormatBlockSize!=0) {
                    uint8_t *buffer = new uint8_t[nFormatBlockSize];
                    if(buffer!=NULL) {
                        uint32 nBufferSize = nFormatBlockSize;
                        ...
                        if(status==FILE_SOURCE_SUCCESS) {
                            if(false == bRawCodecData) {
                                ...
                                //! For backward compatibility, provide normal Codec Data also
                                ...
                                if(status==FILE_SOURCE_SUCCESS) {
                                    track->meta->setData(kKeyRawCodecSpecificData,
                                                         MetaData::TYPE_NONE, buffer,
                                                         nBufferSize);
                                } else {
                                    LOGE(" getFormatBlock failed for raw format\n");
                                }
                            }
                        } else {
                            LOGE(" getFormatBlock failed \n");
                    ...
                    if (!m_pFileSourceWrapper->IsSeekDenied()) {
                        ...
                        for (uint64 lastSyncSampleTime = 0, syncSample = 0;
                            ...
                            if (FILE_SOURCE_DATA_OK != status || bytesRead <= 0){
                                ...
                                //! Set 0ms as thumbnail time for TS
                                //! For TS, only one sync frame at the start is
                                //! used. To make it simpler, set time as 0ms
                                if(FILE_SOURCE_MP2TS != fileFormat)
