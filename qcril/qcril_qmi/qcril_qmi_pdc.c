...
)
{
  ...
  {
    QCRIL_LOG_INFO("unable to find reqlist by event QCRIL_EVT_HOOK_QUERY_MODEM_TEST_MODE");
  }
...
)
{
  ...
  if ( g_pdc_info.cur_deact_sub >= g_pdc_info.sub_count )
  {
    ...
    else
    {
      ...
      {
        QCRIL_LOG_ERROR("unable to find reqlist by event QCRIL_EVT_HOOK_GET_META_INFO");
      }
      g_pdc_info.is_under_list_request = 0;
      g_pdc_info.is_under_processing = 0;
    }
