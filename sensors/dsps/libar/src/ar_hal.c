...
static bool hal_sam_cmc_send_algo_attrib_req()
{
    ...
    req_hdr.msg_id = SNS_SAM_CMC_GET_ATTRIBUTES_REQ_V01;
...
static int hal_sam_cmc_send_batch_req( hal_ar_ctl_t* ar_ctl, uint32_t batch_rate)
{
    sensor1_error_e  error;
    sensor1_msg_header_s req_hdr;
    sns_sam_cmc_batch_req_msg_v01 *sam_req;
    ...
    error = sensor1_alloc_msg_buf( ar_ctl->hndl,
                                   sizeof(sns_sam_cmc_batch_req_msg_v01),
                                   (void**)&sam_req );
    if( SENSOR1_SUCCESS != error ) {
        HAL_LOG_ERROR( "%s: sensor1_alloc_msg_buf() error: %d", __FUNCTION__, error );
        return -1;
    ...
    req_hdr.service_number = CMC_SVC_NUM;
    req_hdr.msg_id = SNS_SAM_CMC_BATCH_REQ_V01;
    req_hdr.msg_size = sizeof( sns_sam_cmc_batch_req_msg_v01 );
    req_hdr.txn_id = 0;
...
static int hal_sam_cmc_send_batch_update_req( hal_ar_ctl_t* ar_ctl, uint32_t batch_rate )
{
    sensor1_error_e error;
    sensor1_msg_header_s req_hdr;
    sns_sam_cmc_update_batch_period_req_msg_v01 *sam_req;
    float batch_rate_in_hz;
    ...
    /* Message Body */
    error = sensor1_alloc_msg_buf( ar_ctl->hndl,
                                   sizeof(sns_sam_cmc_update_batch_period_req_msg_v01),
                                   (void**)&sam_req );
    ...
    req_hdr.service_number = CMC_SVC_NUM;
    req_hdr.msg_id = SNS_SAM_CMC_UPDATE_BATCH_PERIOD_REQ_V01;
    req_hdr.msg_size = sizeof( sns_sam_cmc_update_batch_period_req_msg_v01 );
    req_hdr.txn_id = 0;
...
static bool hal_sam_cmc_send_enable_req( hal_ar_ctl_t* ar_ctl )
{
    ...
    if (hal_wait_for_response( TIME_OUT_MS,
        ...
        HAL_LOG_ERROR("%s: ERROR: No reponse from CMC enable request %d",
                       __FUNCTION__, SNS_SAM_CMC_ENABLE_REQ_V01);
        return (false);
    }
                                            ...
                                            bool single_event )
{
    ...
    sensor1_error_e error;
    sensor1_msg_header_s req_hdr;
    sns_sam_cmc_update_reporting_req_msg_v01* sam_req;
    ...
    /* Message Body */
    error = sensor1_alloc_msg_buf( ar_ctl->hndl,
                                   sizeof(sns_sam_cmc_update_reporting_req_msg_v01),
                                   (void**)&sam_req );
    if( SENSOR1_SUCCESS != error ) {
        HAL_LOG_ERROR("%s: sensor1_alloc_msg_buf() error: %d", __FUNCTION__, error );
        return (false);
    ...
    req_hdr.service_number = CMC_SVC_NUM;
    req_hdr.msg_id = SNS_SAM_CMC_UPDATE_REPORTING_REQ_V01;
    req_hdr.msg_size = sizeof(sns_sam_cmc_update_reporting_req_msg_v01);
    req_hdr.txn_id = 0;

    switch(activity_handle) {
        case HANDLE_IN_VEHICLE:
             handle = SNS_SAM_CMC_MS_VEHICLE_V01;
             break;
        case HANDLE_ON_BICYCLE:
             handle = SNS_SAM_CMC_MS_BIKE_V01;
             break;
        case HANDLE_WALKING:
             handle = SNS_SAM_CMC_MS_WALK_V01;
             break;
        case HANDLE_RUNNING:
             handle = SNS_SAM_CMC_MS_RUN_V01;
             break;
        case HANDLE_STILL:
             handle = SNS_SAM_CMC_MS_STATIONARY_V01;
             break;
        default:
    ...
    switch(event_type) {
        ...
        case ACTIVITY_EVENT_ENTER:
             evt_type = SNS_SAM_CMC_MS_EVENT_REPORT_TYPE_ENTER_V01;
             break;
        case ACTIVITY_EVENT_EXIT:
             evt_type = SNS_SAM_CMC_MS_EVENT_REPORT_TYPE_EXIT_V01;
             break;
        default:
    ...
    if( single_event == true ) {
        sam_req->report_ms_type = SNS_SAM_CMC_MS_REPORT_TYPE_SINGLE_V01;
    }
    else {
        sam_req->report_ms_type = SNS_SAM_CMC_MS_REPORT_TYPE_ALL_V01;
    }
    ...
    if (hal_wait_for_response( TIME_OUT_MS,
        ...
        HAL_LOG_ERROR("%s: ERROR: No reponse for request %d",
                       __FUNCTION__, SNS_SAM_CMC_UPDATE_REPORTING_REQ_V01);
        return (false);
    }
    ...
    if( true != error ) {
        switch( msg_hdr->msg_id ) {
            case SNS_SAM_CMC_ENABLE_RESP_V01:
                HAL_LOG_DEBUG("%s: Received SNS_SAM_CMC_ENABLE_RESP_V01", __FUNCTION__);
            ...
            case SNS_SAM_CMC_CANCEL_RESP_V01:
                HAL_LOG_DEBUG("%s: Received SNS_SAM_CMC_CANCEL_RESP_V01", __FUNCTION__);
                /* Reset instance ID */
                g_ar_control->sam_service[ CMC_SVC_NUM ].instance_id = 0xFF;
                break;
            case SNS_SAM_CMC_GET_ATTRIBUTES_RESP_V01:
                HAL_LOG_DEBUG("%s: Received SNS_SAM_CMC_GET_ATTRIBUTES_RESP_V01", __FUNCTION__);
                ...
                break;
            case SNS_SAM_CMC_BATCH_RESP_V01:
                HAL_LOG_DEBUG("%s: Received SNS_SAM_CMC_BATCH_RESP_V01", __FUNCTION__);
                break;
            case SNS_SAM_CMC_UPDATE_BATCH_PERIOD_RESP_V01:
                HAL_LOG_DEBUG("%s: Received SNS_SAM_CMC_UPDATE_BATCH_PERIOD_RESP_V01", __FUNCTION__);
                break;
            case SNS_SAM_CMC_UPDATE_REPORTING_RESP_V01:
                HAL_LOG_DEBUG("%s: Received SNS_SAM_CMC_UPDATE_REPORTING_RESP_V01", __FUNCTION__);
                break;
            default:
...
static void hal_process_sam_cmc_ind( sensor1_msg_header_s *msg_hdr, void *msg_ptr )
{
    ...
    if (SNS_SAM_CMC_REPORT_IND_V01 == msg_hdr->msg_id) {
        HAL_LOG_DEBUG("%s: SNS_SAM_CMC_REPORT_IND_V01", __FUNCTION__);
        sns_sam_cmc_report_ind_msg_v01* sam_ind =
    ...
    else if(SNS_SAM_CMC_BATCH_IND_V01 == msg_hdr->msg_id) {
        HAL_LOG_DEBUG("%s: SNS_SAM_CMC_BATCH_IND_V01", __FUNCTION__);
        sns_sam_cmc_batch_ind_msg_v01* sam_ind =
            (sns_sam_cmc_batch_ind_msg_v01*)msg_ptr;
        ...
        if (NULL == sample_list) {
            ...
            for(i = 0; i < sam_ind->items_len; i++) {
                ...
                if( (sam_ind->items[ i ].report.motion_state == SNS_SAM_CMC_MS_VEHICLE_V01 &&
                     ((sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_ENTER_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_IN_VEHICLE].enter == true) ||
                      (sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_EXIT_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_IN_VEHICLE].exit == true))) ||

                    (sam_ind->items[ i ].report.motion_state == SNS_SAM_CMC_MS_BIKE_V01 &&
                     ((sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_ENTER_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_ON_BICYCLE].enter == true) ||
                      (sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_EXIT_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_ON_BICYCLE].exit == true))) ||

                    (sam_ind->items[ i ].report.motion_state == SNS_SAM_CMC_MS_WALK_V01 &&
                     ((sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_ENTER_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_WALKING].enter == true) ||
                      (sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_EXIT_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_WALKING].exit == true))) ||

                    (sam_ind->items[ i ].report.motion_state == SNS_SAM_CMC_MS_RUN_V01 &&
                     ((sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_ENTER_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_RUNNING].enter == true) ||
                      (sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_EXIT_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_RUNNING].exit == true))) ||

                    (sam_ind->items[ i ].report.motion_state == SNS_SAM_CMC_MS_STATIONARY_V01 &&
                     ((sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_ENTER_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_STILL].enter == true) ||
                      (sam_ind->items[ i ].ms_event == SNS_SAM_CMC_MS_EVENT_EXIT_V01 &&
                       g_ar_control->hal_track_activity[HANDLE_STILL].exit == true))) ) {
    ...
    for (i = 0; i < count; i++) {
        ...
        switch(curr_sample->data[1]) {
            case SNS_SAM_CMC_MS_EVENT_ENTER_V01:
                 activity_data[i].event_type = ACTIVITY_EVENT_ENTER;
                 break;
            case SNS_SAM_CMC_MS_EVENT_EXIT_V01:
