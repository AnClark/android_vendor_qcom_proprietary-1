...
============================================================================*/
uint32_t Sensor::calcSampleRate(uint64_t ns)
{
    uint32_t sample_rate = 0;
    if (0 == (int)this->max_freq)
        this->max_freq = 1;
    ...
    * rates between "fastest" and 1Hz. If it's slower than 1Hz, request 1Hz */
        ...
        if(sample_rate > (uint32_t)this->max_freq) {
            sample_rate = (uint32_t)this->max_freq;
        }
    }
    else {
        /* default to 1 Hz */
