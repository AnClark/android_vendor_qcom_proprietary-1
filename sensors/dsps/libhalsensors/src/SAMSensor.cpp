...
int SAMSensor::batch(int flags, int64_t period_ns, int64_t timeout)
{
    ...
    if (timeout == 0) {
        batching = false;
        HAL_LOG_DEBUG( "%s:sensor(%s) handle:%d freq:%u report_rate:%u max:%f min:%f",
                        __FUNCTION__, Utility::SensorTypeToSensorString(getType()),
        ...
        HAL_LOG_DEBUG( "%s:sensor(%s) handle:%d freq:%u report_rate_f:%f \
...
bool SAMSensor::processAlgoAttribResp(sensor1_msg_header_s *msg_hdr, void *msg_ptr )
{
    ...
    HAL_LOG_INFO("%s:sensor(%s) Received response %d for svc_num %d handle %d version %d",
                   __FUNCTION__, Utility::SensorTypeToSensorString(getType()),
                   cbrsp_ptr->resp.sns_result_t, msg_hdr->service_number, handle,
                   cbrsp_ptr->algorithm_revision);
                     ...
                     void *msg_ptr)
{
    ...
    case SENSOR1_MSG_TYPE_RESP:
        if (((sns_common_resp_s_v01 *)msg_ptr)->sns_result_t != 0) {
            if(msg_hdr->msg_id == 0x0024) {
                HAL_LOG_ERROR("%s: attributes request failed for handle %d",
                    __FUNCTION__, sam->getHandle());
                sam->setAttribOK(false);
            }
        }
