...
int SMGRSensor::enable(int en)
{
    ...
    if (en) {
        ...
        HAL_LOG_DEBUG("%s:sensor(%s) handle %d, freq=%d report_rate=%d batched=%d",
        ...
        if (true != err) {
            HAL_LOG_ERROR("%s:sensor(%s) Failed for handle %d @ samp %d Hz rpt %d Hz batched %d",
                __FUNCTION__, Utility::SensorTypeToSensorString(getType()), handle, freq, report_rate, batching);
...
int SMGRSensor::batch(int flags, int64_t period_ns, int64_t timeout)
{
    ...
    HAL_LOG_DEBUG( "%s:sensor(%s) sample_rate=%uHz report_rate_f=%fHz curr sample rate:%u cur rpt rate:%u max:%f min:%f",
...
===========================================================================*/
bool SMGRSensor::SMGRPrepareAddMsg(uint32_t sample_rate,
...
===========================================================================*/
bool SMGRSensor::SMGRReportAdd(uint32_t sample_rate, uint32_t report_rate,
                            bool buffer, bool wait_for_resp)
{
    ...
    HAL_LOG_DEBUG("%s:sensor(%s) handle=%d, sample_rate=%d report_rate=%d buffer=%d",
