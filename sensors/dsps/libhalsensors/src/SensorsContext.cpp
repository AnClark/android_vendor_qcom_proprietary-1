...
int SensorsContext::poll(sensors_event_t* data, int count)
{
    int i = 0;
    int err;
    bool wakeup_event_in_queue = false;
    ...
    while(i < count) {
        ...
        if(mSensors[data[i].sensor]->getFlags() & SENSOR_FLAG_WAKE_UP){
            wakeup_event_in_queue = true;
        }
        i++;
    }

    /* Release wakelock if held */
    if (wakeup_event_in_queue == false && data_cb->sensors_wakelock_held == true) {
...
void SensorsContext::addSensor(int handle)
{
    ...
    switch(handle){
        ...
        case HANDLE_GESTURE_TAP:
            property_get( HAL_PROP_TAP, add_sensor, "true" );
            if (!strncmp("true", add_sensor, 4)) {
        ...
        case HANDLE_GESTURE_TILT:
            property_get( HAL_PROP_TILT, add_sensor, "true" );
            if (!strncmp("true", add_sensor, 4) && Utility::isGyroAvailable()) {
        ...
        case HANDLE_ORIENTATION_WAKE_UP:
            ...
            break;
        case HANDLE_PAM:
            property_get( HAL_PROP_PAM, add_sensor, "true" );
            if (!strncmp("true", add_sensor, 4)) {
...
int SensorsContext::getSensorList()
{
    ...
    if (Utility::waitForResponse(TIME_OUT_MS * 10,
