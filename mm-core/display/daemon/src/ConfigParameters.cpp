...
int32_t SVIHWParams::ParseNonArrayParameters() {
    ...
    ret = GetIntegerValue(SVI_EN_POWER_SAVE, &tmp);
    if (ret)
        return ret;
    mEnablePowerSave = tmp > 0 ? tmp : 0;
...
void SVIHWParams::PrintParsedParameters() {
    ...
    LOGD("Enable Power Save______________= %u", mEnablePowerSave);
...
/* TO DO: Discuss if these need to be read from XML as well */
    uint32_t mFilterSpeed = ABA_FILTER_SPEED_MEDIUM;
    uint32_t mLengthBrightMappingTable = 8;
    uint32_t pBrightMappingFactor[]    = { 9381, 4750, 3472, 2919, 2502, 2198,
                                           1842,  1642 };
    uint32_t pBrightMappingLux[]       = {  300,  750, 1250, 1750, 2500, 3500,
                                           6000, 10000 };
...
config_start:
    cp->uContentBoostingFactor              = mContrastStrength;
    cp->uLuxBoostingFactor                  = mContrastStrength;
    cp->uTemporalFilterFactor               = mFilterSpeed;
    cp->uDarkPixBrightnessGain              = mBrightnessStrength;
    cp->uBrightPixBrightnessGain            = mBrightnessStrength;

    //LUX mapping table info
    ...
    cp->uBrightnessMappingTableLength       = mLengthBrightMappingTable;

    if (CopyArray(&cp->pBrightnessStrengthMappingFactor, pBrightMappingFactor,
            mLengthBrightMappingTable) ||
        CopyArray(&cp->pBrightnessStrengthMappingLux, pBrightMappingLux,
            mLengthBrightMappingTable))
        XMLParseError(ERROR_NO_MEMORY);
    ...
    cp->bEnableBacklightReduction           = mEnablePowerSave;
