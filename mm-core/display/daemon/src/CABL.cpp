...
int CABL::Notify(int notification_type) {
    ...
    pthread_mutex_lock(&mCABLLock);
    if (mHistStatus == 0 && (notification_type == NOTIFY_TYPE_BL_UPDATE ||
            notification_type == NOTIFY_TYPE_UPDATE)) {
...
int32_t CABL::start_cabl() {
    ...
    long init_level = (mUserBLLevel >= mOEMParams.bl_min_level) ?
            (long) mUserBLLevel : (long) mOEMParams.bl_min_level;
