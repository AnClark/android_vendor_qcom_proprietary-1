...
void rtspSink::applySettings(rtspParams *parsed)
{
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB ::parsed wfd = %lu",(parsed->state.wfd.getValidWfd()).to_ulong());
    ...
    if (parsed->mesg.cmd == setParameterCmd &&
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB ::parsed isect wfd = %lu",(parsed->state.wfd.getValidWfd()).to_ulong());
