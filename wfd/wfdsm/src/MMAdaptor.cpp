...
uint64 MMAdaptor::getCurrentPTS() {
    ...
    if (mmCreated) {
        ...
        LOGD("Calling wfd_mm_get_current_PTS() done.  ret=%d , timestamp = %llu", ret , timeStamp);
        if (ret==WFD_STATUS_SUCCESS) {
            return timeStamp;
        }
    } else {
...
void MMAdaptor::event_update_cb(WFD_MM_Event_t event,
    ...
    case WFD_EVENT_MM_AUDIO:
        ...
        {
            int numChannels = 2;//default
            if(pEvtData)
            {
                numChannels = *(static_cast<int*>(pEvtData));
            }
            eventMMUpdate(MM_AUDIO_PROXY_DEVICE_OPENED,convertWfdToMMStatus(status),numChannels,0);
        }
