...
int disableWfd ()
{
    ...
    {
     MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"disableWfd: failed to destroy mutex");
    }
    m_pMutex = NULL;
    gSurface = NULL;
...
static void singlestring_callback(const char* eName, const char* str_argument)
{
    ...
    if (venc_mutex_lock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR," singlestring_callback : Failed to Acquire Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR," singlestring_callback : Acquiring Mutex" );
    ...
    if (venc_mutex_unlock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR," singlestring_callback : Failed to Release Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR," singlestring_callback : Releasing Mutex" );
}
...
{
    if (venc_mutex_lock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR," eventSessionStateChanged : Failed to Acquire Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH," eventSessionStateChanged : Acquiring Mutex" );
    ...
    default:
        ...
        if (venc_mutex_unlock(m_pMutex) != 0)
         MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR," eventSessionStateChanged : Failed to Release Mutex" );
        else
         MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH," eventSessionStateChanged : Releasing Mutex" );
        return;
    }
    ...
    if (venc_mutex_unlock(m_pMutex) != 0)
       ...
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR," eventSessionStateChanged : Releasing Mutex" );
...
void eventStreamControlCompleted(int cmdType, int rtspSessId)
{
    size_t len;
    if (venc_mutex_lock(m_pMutex) != 0)
         MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventStreamControlCompleted : Failed to Acquire Mutex" );
     else
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventStreamControlCompleted : Acquiring Mutex" );
    ...
    if (venc_mutex_unlock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventStreamControlCompleted : Failed to Release Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"eventStreamControlCompleted : Releasing Mutex" );
...
{
    if (venc_mutex_lock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventUIBCEnabled : Failed to Acquire Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventUIBCEnabled : Acquiring Mutex" );
    snprintf(strarray[0],256, "%d", rtspSessId);
    ...
    if (venc_mutex_unlock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventUIBCEnabled : Failed to Release Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"eventUIBCEnabled : Releasing Mutex" );
}

void eventUIBCDisabled(int rtspSessId)
{
    if (venc_mutex_lock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventUIBCDisabled : Failed to Acquire Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventUIBCDisabled : Acquiring Mutex" );
    snprintf(strarray[0],256, "%d", rtspSessId);
    ...
    if (venc_mutex_unlock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventUIBCDisabled : Failed to Release Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"eventUIBCDisabled : Releasing Mutex" );
}

void eventUIBCRotate(int angle)
{
    if(venc_mutex_lock(m_pMutex) !=0 )
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventUIBCRotate: Failed to acquire mutex");
...
{
    if (venc_mutex_lock(m_pMutex) != 0)
        MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventMMUpdate : Failed to Acquire Mutex" );
    else
       MM_MSG_PRIO(MM_GENERAL,MM_PRIO_HIGH,"eventMMUpdate : Acquiring Mutex" );
  ...
  case MM_RTP_EVENT_RTCP_RR_MESSAGE:
      {
          ...
          if(!evtData1 || !evtData2) {
              ...
              if (venc_mutex_unlock(m_pMutex) != 0)
                  MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventMMUpdate : Failed to Release Mutex" );
              else
                 MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"eventMMUpdate : Releasing Mutex" );
              return;
          }
