...
inline bool IS_PROXY_IN_USE() {
  if(((int)(android::AudioSystem::getDeviceConnectionState((audio_devices_t)
  AUDIO_DEVICE_OUT_BLUETOOTH_A2DP,""))
      == (int)android_audio_legacy::AudioSystem::DEVICE_STATE_AVAILABLE) ||
  ((int)(android::AudioSystem::getDeviceConnectionState((audio_devices_t)
      ...
      void *appData)
  {
      ...
      getCfgItem(AUDIO_AV_SYNC_DEL_KEY,(int*)(&m_nAudioDelay));
      {
          MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
              "AudioSource:: Audio delay %ld", m_nAudioDelay);
      }
             ...
             else
             {
               if(pThis->m_pEventHandlerFn)
               {
                 pThis->m_pEventHandlerFn(pThis->m_appData, pThis->m_nModuleId,
                                         WFDMMSRC_AUDIO_PROXY_OPEN_DONE, OMX_ErrorNone,
                                         pThis->m_PcmConfig.channels);
               }
             }
           }
           else
            ...
            if(pThis->m_eAudioCodingType == OMX_AUDIO_CodingPCM)
            {
                ...
                if(OMX_FALSE == bInsertSilence)
                {
                  ...
                  */
                  ...
                  for(OMX_U32 count = 0; count < (nCopyCount >> 1); count++)
                  {
                      nVal = *pSrc++;
                      *pDst++ = (nVal >> 8) | (nVal << 8);
                  }
                      ...
                      if(OMX_TRUE == m_bIsProxyAvailabile)
                      {
                          ...
                          if(m_pEventHandlerFn)
                          {
                              m_pEventHandlerFn(m_appData, m_nModuleId,
                                                ...
                                                OMX_ErrorNone,m_PcmConfig.channels);
                          }
                      }
