...
static int get_version_info(void)
{
	int err = 0, ret, i, count = 0;
	unsigned char *temp_version_table_p;
	char image_index[3];

	/* Allocating memory for retrieving version table from smem (sysfs entries) */
	version_table_p = malloc(VERSION_TABLE_S);
	if (!version_table_p) {
		ALOGE("Unable to allocate memory for version table");
		return -1;
	}
	memset(version_table_p, 0, VERSION_TABLE_S);

	temp_version_table_p = version_table_p;
...
PACK(void*) diagpkt_version_handler(PACK(void*) req_ptr, uint16 pkt_len)
{
	...
	*/
	rsp = diagpkt_subsys_alloc(VERSION_DIAGPKT_SUBSYS,
                VERSION_DIAGPKT_PREFIX, VERSION_TABLE_S - 2);

        if ((NULL != rsp) && (pkt_len == 4)) {
                /* copy request packet first */
                memcpy((char *)rsp, req_ptr, pkt_len);
                /* now copy the image version info */
                memcpy((char *)rsp+pkt_len, version_table_p,
                        VERSION_TABLE_S - 2 - pkt_len);
        } else {
...
int main()
{
	...
	DIAGPKT_DISPATCH_TABLE_REGISTER_V2(VERSION_DIAGPKT_PROCID,
