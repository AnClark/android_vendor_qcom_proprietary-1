...
static int isp_pipeline_start_common(isp_pipeline_t *pipeline)
{
  ...
  /* Update all modules before start stream */
  pipeline->hfr_update_batch1 = 0xFFFFFFFF;
  pipeline->hfr_update_batch2 = 0xFFFFFFFF;
  pipeline->hfr_update_mod_mask = 0xFFFFFFFF;
  ...
  /*For HFR mode divide modules into batches. Batch 1,2 can be configured here.
    ...
    */
  if (pipeline->pix_params.cfg_and_3a_params.cfg.camif_cfg.hfr_mode ==
      CAM_HFR_MODE_120FPS) {
     pipeline->hfr_update_batch1 = (1 << ISP_MOD_ROLLOFF) | (1 << ISP_MOD_LA);
     pipeline->hfr_update_batch2 = ~(pipeline->hfr_update_batch1);
     pipeline->hfr_update_mod_mask = pipeline->hfr_update_batch1;
  } else
    pipeline->hfr_update_mod_mask = 0xFFFFFFFF;
