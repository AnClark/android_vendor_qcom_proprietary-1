...
int isp_hw_proc_hw_update(void *ctrl, struct msm_isp_event_data *sof)
{
  ...
  if (rc < 0) {
    CDBG_ERROR("%s: Error in HW update lock, frame id = %u, rc = %d\n",
      __func__, hw_frame_id, rc);
    return 0;
  }
  else {
    /* pipeline hw update */
    ...
    cfg_cmd.cmd_len = sizeof(hw_frame_id);
    cfg_cmd.num_cfg = 1;
    reg_cfg_cmd[0].cmd_type = VFE_HW_UPDATE_UNLOCK;
    rc = ioctl(isp_hw->fd, VIDIOC_MSM_VFE_REG_CFG, &cfg_cmd);
    if (rc < 0) {
      CDBG_ERROR("%s: Error in HW update unlock, frame id = %d, rc = %d\n",
        __func__, hw_frame_id, rc);
      rc = 0;
    }
