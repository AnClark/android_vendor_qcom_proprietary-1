...
static boolean af_port_send_legacy_status_update(mct_port_t *port)
{
  ...
  CDBG_ERROR("%s: Send AF Status Update! af_state=%d af_status=%d",
    __func__,af_port->af_trans.af_state, af_port->af_status);
  /* update AF status */
  ...
  if (af_port->af_trans.af_state == CAM_AF_STATE_INACTIVE) {
